...
FastKey is a desktop utility for mapping HID device buttons to custom hotkeys.
...
- Some Bluetooth devices expose audio or hands-free control instead of standard input events, so FastKey cannot see them.
...
- Installer script: `F:\CODE\FastKey\FastKeySetup.iss`
...
1. Compile `FastKey.ahk` into `FastKey.exe` with AutoHotkey v2.
2. Open `FastKeySetup.iss` in Inno Setup.
